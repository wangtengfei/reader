...
public class ZLAndroidWidget extends View implements ZLViewWidget, View.OnLongClickListener
{
	...
	void drawOnBitmap(Bitmap bitmap, ZLView.PageIndex index)
	{
		...
		final ZLAndroidPaintContext context = new ZLAndroidPaintContext(new Canvas(bitmap), getWidth(), getMainAreaHeight(), view.isScrollbarShown() ? getVerticalScrollbarWidth()
				: 0);
	...
	private void drawFooter(Canvas canvas)
	{
		...
		{
			System.out.println("myFooterBitmap");
			// Bitmap immutableBitmap =
			// BitmapFactory.decodeResource(getResources(),
			// R.drawable.book_progress_bg);
			// myFooterBitmap = immutableBitmap.copy(Bitmap.Config.RGB_565,
			// true);
		...
		canvas.drawBitmap(myFooterBitmap, 0, 0, myPaint);
	}

	private void onDrawStatic(Canvas canvas)
	{
		myBitmapManager.setSize(getWidth(), getMainAreaHeight());
		canvas.drawBitmap(myBitmapManager.getBitmap(ZLView.PageIndex.current), 0, Const.TITLE_HEIGHT, myPaint);
		drawFooter(canvas);
	}
