...
public final class ZLSQLiteConfig extends ZLConfig
{
	...
	{
		// myDatabase = context.openOrCreateDatabase("config.db",
		// Context.MODE_PRIVATE, null);
		myDatabase = SQLiteDatabase.openOrCreateDatabase("/data/data/com.rdweiba.main/config.db", null);
		 ...
		 * "SELECT groupName,name FROM config WHERE groupName LIKE ? GROUP BY name"
		 * , new String[] { "/%" }); while (cursor.moveToNext()) {
	...
	{
		final LinkedList<String> list = new LinkedList<String>();
		final Cursor cursor = myDatabase.rawQuery("SELECT name FROM config WHERE groupName = ?", new String[]
		{ group });
		while (cursor.moveToNext())
		{
			list.add(cursor.getString(0));
