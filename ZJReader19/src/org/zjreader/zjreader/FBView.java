...
import org.zjreader.bookmodel.FBHyperlinkType;
...
public final class FBView extends ZLTextView
{
	...
	public FBView(ZJReaderApp reader)
	{
		super(reader);
		myReader = reader;
	}
	...
	private TapZoneMap getZoneMap()
	{
		...
		final String id = ScrollingPreferences.Instance().HorizontalOption.getValue() ? "right_to_left" : "up";
	...
	public boolean onFingerSingleTap(int x, int y)
	{
		...
		myReader.doActionWithCoordinates(
				getZoneMap().getActionByCoordinates(x, y, myContext.getWidth(), myContext.getHeight(),
	...
	public boolean onFingerDoubleTap(int x, int y)
	{
		...
		myReader.doActionWithCoordinates(getZoneMap().getActionByCoordinates(x, y, myContext.getWidth(), myContext.getHeight(), TapZoneMap.Tap.doubleTap), x, y);
		return true;
	}
	...
	{
		final ScrollingPreferences.FingerScrolling fingerScrolling = ScrollingPreferences.Instance().FingerScrollingOption.getValue();
	...
	public boolean onFingerRelease(int x, int y)
	{
		...
		if (isFlickScrollingEnabled())
		{
			myReader.getViewWidget().startAnimatedScrolling(x, y, ScrollingPreferences.Instance().AnimationSpeedOption.getValue());
			return true;
		}
	...
	public boolean onFingerLongPress(int x, int y)
	{
		...
		if (region != null)
		{
			...
			{
				doSelectRegion = myReader.ImageTappingActionOption.getValue() != ZJReaderApp.ImageTappingAction.doNothing;
	...
	public boolean onFingerMoveAfterLongPress(int x, int y)
	{
		...
		if (region != null)
		{
			...
			{
				if (myReader.WordTappingActionOption.getValue() != ZJReaderApp.WordTappingAction.doNothing)
				{
	...
	public boolean onFingerReleaseAfterLongPress(int x, int y)
	{
		...
		if (region != null)
		{
			...
			if (soul instanceof ZLTextWordRegionSoul)
			{
				doRunAction = myReader.WordTappingActionOption.getValue() == ZJReaderApp.WordTappingAction.openDictionary;
			} else if (soul instanceof ZLTextImageRegionSoul)
			{
				doRunAction = myReader.ImageTappingActionOption.getValue() == ZJReaderApp.ImageTappingAction.openImageView;
			}
	...
	public boolean onTrackballRotated(int diffX, int diffY)
	{
		...
		ZLTextRegion region = getSelectedRegion();
		final ZLTextRegion.Filter filter = (region != null && region.getSoul() instanceof ZLTextWordRegionSoul) || myReader.NavigateAllWordsOption.getValue() ? ZLTextRegion.AnyRegionFilter
	...
	public ZLColor getTextColor(ZLTextHyperlink hyperlink)
	{
		...
		case FBHyperlinkType.INTERNAL:
			return myReader.Model.Book.isHyperlinkVisited(hyperlink.Id) ? profile.VisitedHyperlinkTextOption.getValue() : profile.HyperlinkTextOption.getValue();
	...
	private class Footer implements FooterArea
	{
		...
		{
			final ZJReaderApp reader = myReader;
			if (reader == null)
			{
				return;
			}
			...
			final ZLColor fgColor = reader.getColorProfile().WhiteTextOption.getValue();
			...
			context.setFont(reader.FooterFontOption.getValue(), 20, false, false, false);
			...
			if (reader.FooterShowProgressOption.getValue())
			{
				...
				String progress = df.format(pagesProgress * 100.00 / bookLength) + "%";
			...
			context.drawString(Math.round((right - infoWidth) / 2), 18, infoString);
			...
			// myGaugeWidth * pagesProgress / bookLength);
			//
			// context.setFillColor(fillColor);
			...
			// final int xCoord = left + 2 * lineWidth + (int) (1.0 *
			// myGaugeWidth * refCoord / fullLength);
