...
import com.rdweiba.main.pagesview.OnImageRenderedListener;
import com.rdweiba.main.pagesview.PagesProvider;
import com.rdweiba.main.pagesview.RenderingException;
import com.rdweiba.main.pagesview.Tile;
...
public class PDFPagesProvider extends PagesProvider
{
	...
	/*
	 * render a little more than twice the screen height, so the next page will
	 * be ready
	 */
	...
	{
		int maxMax = (int) (7 * 1024 * 1024) + this.extraCache; /*
																 * at most
																 * allocate this
																 * much unless
																 * absolutely
	...
	/**
	 * Smart page-bitmap cache. Stores up to approx maxCacheSizeBytes of images.
	 * Dynamically drops oldest unused bitmaps. TODO: Return high resolution
	 * bitmaps if no exact res is available. Bitmap images are tiled - tile size
	 * is specified in PagesView.TILE_SIZE.
	 */
	...
	private static class BitmapCache
	{
		...
		Bitmap get(Tile k)
		{
			...
			{
				Log.d("cx.hell.android.pdfview.pagecache", "hits: " + hits + ", misses: " + misses + ", hit ratio: " + (float) (hits) / (float) (hits + misses) + ", size: "
						+ this.bitmaps.size());
			}
			return b;
		}
		 ...
		 * 
		 * @param tile
		 *            tile definition (page, position etc), cache key
		 * @param bitmap
		 *            rendered tile contents, cache value
		 */
		...
		/**
		 * Check if cache contains specified bitmap tile. Doesn't update
		 * last-used timestamp.
		 * 
		...
		/**
		 * Estimate bitmap memory size. This is just a guess.
		 */
		...
		/**
		 * Determine if adding this bitmap would grow cache size beyond max
		 * size.
		 */
		private synchronized boolean willExceedCacheSize(Bitmap bitmap)
		{
			return (this.getCurrentCacheSize() + BitmapCache.getBitmapSizeInCache(bitmap) > maxCacheSizeBytes);
		}
	...
	private static class RendererWorker implements Runnable
	{
		...
		/**
		 * Used as a synchronized flag. If null, then there's no designated
		 * thread to render tiles. There might be other worker threads running,
		 * but those have finished their work and will finish really soon. Only
		 * this one should pick up new jobs.
		 */
		 ...
		 * 
		 * @param pdfPagesProvider
		 *            parent pages provider
		 */
		...
		/**
		 * Called by outside world to provide more work for worker. This also
		 * starts rendering thread if one is needed.
		 * 
		 * @param tiles
		 *            a collection of tile objects, they carry information about
		 *            what should be rendered next
		 */
		synchronized void setTiles(Collection<Tile> tiles, BitmapCache bitmapCache)
		{
			...
			{
				// Log.i(TAG,
				// "setTiles notices tiles is not empty, that means RendererWorkerThread exists and there's no need to start new one");
			}
		}

		/**
		 * Get tiles that should be rendered next. May not block. Also sets
		 * this.workerThread to null if there's no tiles to be rendered
		 * currently, so that calling thread may finish. If there are more tiles
		 * to be rendered, then this.workerThread is not reset.
		 * 
		 * @return some tiles
		 */
		synchronized Collection<Tile> popTiles()
		{
			if (this.tiles == null || this.tiles.isEmpty())
			{
				this.workerThread = null; /*
										 * returning null, so calling thread
		...
		/**
		 * Thread's main routine. There might be more than one running, but only
		 * one will get new tiles. Others will get null returned by
		 * this.popTiles and will finish their work.
		 */
		public void run()
		{
			while (true)
	...
	public PDFPagesProvider(Activity activity, PDF pdf, boolean gray, boolean skipImages, boolean doRenderAhead, Handler handler)
	{
		this.gray = gray;
		this.pdf = pdf;
		this.omitImages = skipImages;
	...
	/**
	 * Render tiles. Called by worker, calls PDF's methods that in turn call
	 * native code.
	 * 
	 * @param tiles
	 *            job description - what to render
	 * @return mapping of jobs and job results, with job results being Bitmap
	 *         objects
	 */
	...
	private Bitmap renderBitmap(Tile tile) throws RenderingException
	{
		...
		{
			/*
			 * last minute check to make sure some other thread hasn't rendered
			 * this tile
			 */
			...
			pagebytes = pdf.renderPage(tile.getPage(), tile.getZoom(), tile.getX(), tile.getY(), tile.getRotation(), gray, omitImages, size); /* native */
			...
			if (gray)
			{
				Bitmap b = Bitmap.createBitmap(pagebytes, size.width, size.height, Bitmap.Config.ARGB_8888);
			...
			{
				Bitmap b = Bitmap.createBitmap(pagebytes, size.width, size.height, Bitmap.Config.RGB_565);
	 ...
	 * @param tile
	 *            which bitmap
	 * @return rendered tile; tile represents rect of TILE_SIZE x TILE_SIZE
	 *         pixels, but might be of different size (should be scaled when
	 *         painting)
	 */
	@Override
	...
	public int getPageCount()
	{
		...
		if (c <= 0)
			throw new RuntimeException("failed to load pdf file: getPageCount returned " + c);
		return c;
	}
	...
	/**
	 * View informs provider what's currently visible. Compute what should be
	 * rendered and pass that info to renderer worker thread, possibly waking up
	 * worker.
	 * 
	 * @param tiles
	 *            specs of whats currently visible
	 */
